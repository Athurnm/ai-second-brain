...
Scan this workspace for things that are still open — promises made, replies
still owed, people waiting on the user — and produce a follow-up list.
Draft messages where useful, but **never send or post anything** without
the user explicitly approving each one.
