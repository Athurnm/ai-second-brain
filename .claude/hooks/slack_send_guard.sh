...
try:
    ...
    reason = (f"APPROVAL GATE - Slack send to [{ch}]. Did the owner explicitly say kirim/approve for THIS exact message? "
              f'Preview: "{txt}"')
except Exception:
    reason = "APPROVAL GATE - Slack send detected. Confirm the owner explicitly approved this exact message (kirim)."
...
printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"ask","permissionDecisionReason":"APPROVAL GATE - Slack send detected. Confirm the owner explicitly approved this exact message (kirim)."}}\n'
exit 0
