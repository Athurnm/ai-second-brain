#!/usr/bin/env python3
"""Headless two-step GA4 OAuth: `auth-url` prints the consent URL and persists the PKCE
verifier; `auth-save --code` exchanges the pasted code and writes token_ga4_work.json.
Reuses the Work Drive OAuth client (same credentials.json + localhost:8080 redirect).
Sign in as you@yourcompany.com (the account with GA access)."""
...
TOKEN_FILE = os.path.join(SCRIPT_DIR, '..', 'token_ga4_work.json')
STATE_FILE = os.path.join(SCRIPT_DIR, '.ga4_auth_state.json')
